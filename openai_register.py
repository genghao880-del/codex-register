...
def request_with_retry(
    ...
    **kwargs,
    ...
    """
    Wraps curl_cffi.requests calls with retry logic for handling "curl: (35)" errors.
    """
        ...
        try:
            ...
            if "curl: (35)" in str(e):
                if attempt < max_retries:
                    print(f"[Warning] 遇到 'curl: (35)' 错误，{retry_delay} 秒后重试... (第 {attempt + 1}/{max_retries} 次)")
                    time.sleep(retry_delay)
                    continue
            raise e
    raise requests.RequestsError(f"重试次数已耗尽: {url}")
...
def run(proxy: Optional[str]) -> Optional[str]:
    ...
    try:
        trace = request_with_retry(
            "GET", "https://cloudflare.com/cdn-cgi/trace", session=s, timeout=10
        )
        trace = trace.text
    ...
    try:
        resp = request_with_retry("GET", url, session=s, timeout=15)
        ...
        signup_resp = request_with_retry(
            "POST",
        ...
        if signup_resp.status_code != 200:
            print(f"[-] 注册表单提交失败，跳过本次注册。")
            return None

        otp_resp = request_with_retry(
            "POST",
        ...
        create_account_resp = request_with_retry(
            "POST",
        ...
        select_resp = request_with_retry(
            "POST",
        ...
        for _ in range(6):
            final_resp = request_with_retry(
                "GET", current_url, session=s, allow_redirects=False, timeout=15
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--sleep-max", type=int, default=10, help="循环模式最长等待秒数"
    )
